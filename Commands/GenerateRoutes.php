...
class GenerateRoutes extends BaseCommand
{
    protected $group       = 'custom';
    protected $name        = 'generate:routes';
    protected $description = 'Ajoute automatiquement les routes pour un contrôleur.';

    public function run(array $params)
    {
        if (empty($params)) {
            CLI::error("❌ Veuillez spécifier le nom du contrôleur.");
            return;
        }
        ...
        $routesToAdd = [
            "\$routes->get('$params[0]', '$controllerName::index');",
            "\$routes->get('$params[0]/show/(:num)', '$controllerName::show/\$1');",
            "\$routes->get('$params[0]/create', '$controllerName::create');",
            "\$routes->post('$params[0]/store', '$controllerName::store');",
            "\$routes->get('$params[0]/edit/(:num)', '$controllerName::edit/\$1');",
            "\$routes->post('$params[0]/update/(:num)', '$controllerName::update/\$1');",
            "\$routes->get('$params[0]/delete/(:num)', '$controllerName::delete/\$1');",
        ];
        ...
        if ($routesAlreadyExist) {
            CLI::error("⚠️  Les routes pour '$params[0]' existent déjà.");
            return;
        }

        // Ajouter les routes à la fin du fichier `Routes.php`
        $newRoutes = "\n// Routes pour $params[0]\n" . implode("\n", $routesToAdd) . "\n";
        file_put_contents($routeFilePath, $routeFileContent . $newRoutes);

        CLI::write("✅ Routes ajoutées avec succès pour '$params[0]' dans app/Config/Routes.php", 'green');
    }
}
*/
