...
<form method="post" action="<?= site_url('Permis/store/') ?>">
	...
	<label>Numéro</label>
	<input type='text' pattern="^[0-9A-Z]{9}$" oninput="setUpper(document.getElementById('num_permis'));" id='num_permis' name='num_permis' value='<?= isset($item) ? $item->num_permis : '' ?>' class='form-control' required>
